...
export function activate(context) {
    ...
    const url = config.get('url', 'http://127.0.0.1:5001/mcp');
    const apiKey = config.get('apiKey', '');
    const defaultModel = config.get('defaultModel', 'deepseek-chat');
    const modelsProvider = new ModelsProvider(url, apiKey);
    const chatPanelProvider = new ChatPanelProvider(url, apiKey, defaultModel);
    context.subscriptions.push(vscode.window.registerTreeDataProvider('ds2api-mcp-cursor.models', modelsProvider), vscode.window.registerWebviewViewProvider('ds2api-mcp-cursor.chat', chatPanelProvider), vscode.commands.registerCommand('ds2api-mcp-cursor.showStatus', async () => {
        const ok = await checkConnection(url);
        vscode.window.showInformationMessage(ok ? `✅ ds2api connected at ${url}` : `❌ Cannot reach ds2api at ${url}`);
    ...
    if (config.get('autoInjectRules', true)) {
