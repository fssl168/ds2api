...
export class ClaudeCodeClient {
    ...
    async callTool(name, args = {}) {
        ...
        try {
            ...
            if (!resp.ok) {
                return { content: [{ type: "text", text: `HTTP ${resp.status}: ${JSON.stringify(data)}` }], isError: true };
            }
            if (data.error) {
                return { content: [{ type: "text", text: `MCP Error (${data.error.code}): ${data.error.message}` }], isError: true };
            }
            return data.result || { content: [{ type: "text", text: "Empty response" }] };
        }
        finally {
            clearTimeout(timer);
        }
