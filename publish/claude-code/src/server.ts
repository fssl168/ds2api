...
function createClaudeCodeServer(options?: ClaudeCodeOptions): McpServer {
  ...
  server.tool(
    "chat",
    `Send chat completion to ds2api for Claude Code.
...
- Use qwen-coder for code generation`,
    ...
    async (params) => {
      try {
        ...
        return { content: [{ type: "text" as const, text: error.message || String(error) }], isError: true };
      }
    }
  );
  ...
  server.tool(
    "list_models",
    "List all AI models available through ds2api (DeepSeek + Qwen)",
    {},
    async () => {
      try {
  ...
  server.tool(
    "get_status",
    "Check ds2api service health, account pool status, and endpoint info",
    {},
    async () => {
      try {
  ...
  server.tool(
    "get_pool_status",
    "Monitor ds2api account pool utilization (DeepSeek PoW/WASM + Qwen Acquire/Release)",
    {
  ...
  server.tool(
    "embeddings",
    "Generate text embeddings via ds2api",
    {
...
export async function startClaudeCodeStdio(options?: ClaudeCodeOptions): Promise<void> {
